-- Rewrite FL-Attack project page: corrects the HE ("no encryption executed")
-- and DP flatness (sigma*sqrt(d) noise dominance) mislabeling from the prior
-- copy, retires the dead poster_1080p.png cover, and updates tech_tags to
-- reflect the differential-privacy framing over the old generic
-- privacy-by-design tag.
update public.projects
set
  description = $desc$Gradient inversion benchmark that rebuilds recognizable CelebA faces from one federated client's raw update, plus an honest accounting of what its differential privacy and encryption rows actually measure.$desc$,
  full_description = $fd$Federated learning is sold on a simple promise: your data stays on your device, only gradients leave it. This repo tests what that promise is worth. It trains a small CNN across simulated clients on CelebA, captures one client's gradient update from a single round, and runs a DLG/iDLG-style optimization attack against it. The reconstruction comes back at 29.38 dB PSNR and 0.920 SSIM, which is a recognizable photograph of a person who never uploaded a photograph.
...
It matters for reading the defense numbers later. Local differential privacy (clip the client's release, add calibrated Gaussian noise before it goes out) is the mechanism that belongs at this exact release point, so evaluating it here is fair. Secure aggregation is not, because there is no aggregate yet.
...
I ran the undefended attack once per client across all ten clients, same config each time.
...
The experiment I actually want is still unrun. The pieces are already in the codebase: `capture_mode='agg_update'` captures the FedAvg-averaged update instead of a single client's gradient, and `differential_privacy.aggregate_clipped_noisy` applies central DP at that point. Run together, they model what a curious aggregator sees under real secure aggregation, which is the threat model people have in mind when they say "we use HE." No GPU or CelebA compute was available this cycle, so that row stays empty rather than filled with a guess.
...
Full architecture diagram (10425px wide, open it in its own tab): https://raw.githubusercontent.com/imaddde867/FL-Attack/main/architecture_mermaid.png$fd$,
  tech_tags = ARRAY['federated-learning','gradient-inversion','differential-privacy','security-evaluation','pytorch'],
  image_url = $img$https://raw.githubusercontent.com/imaddde867/FL-Attack/main/results/report/figures/dashboard_hero.png$img$,
  updated_at = now()
